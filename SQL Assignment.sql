...
- - 2. Total number of distinct records for each of the keys listed below:
...
- - 3. If there any columns with null values in the Users table
SELECT COUNT(*)
FROM user
WHERE id IS NULL OR name IS NULL OR review_count IS NULL OR yelping_since IS NULL ORuseful IS NULL OR funny IS NULL OR cool IS NULL OR fans IS NULL OR average_stars IS NULL OR compliment_hot IS NULL OR compliment_more IS NULL OR compliment_profile IS NULL OR compliment_cute IS NULL OR compliment_list IS NULL OR compliment_note IS NULL OR compliment_plain IS NULL OR compliment_cool IS NULL OR compliment_funny IS NULL OR compliment_writer IS NULL OR compliment_photos IS NULL 
- - Answer: "no"

- - 4. Minimum, maximum, and average value for the following fields:
SELECT AVG(Stars)
FROM Review;
ELECT AVG(Stars)
FROM Business;
...
The 4-5 star group seems to have shorter hours then the 2-3 star group. The query only returned three businesses so its not a great sample size
...
- - 3.
	i. 	Type of analysis you chose to do: Predicting whether a business will stay open or close. We wish not to explicitly examine the text of the reviews, but this would be an interesting analysis.
	
	ii.	Write 1-2 brief paragraphs on the type of data you will need for your analysis and why you chose that data: To better assist businesses in understanding the relevance of several elements that will help them stay open. Some information that may be useful: number of reviews, business star rating, hours of operation, and, of course, location location location. We'll collect the latitude and longitude, as well as the city, state, and country. To make processing easier later on, include your postal code and address. Classifications and characteristics will be utilized to differentiate between various sorts of 'is open' determines which firms are open and which are closed. been permanently closed (not for a few hours).
	
	iii. 	Provide the SQL code you used to create your final dataset:
SELECT B.id,B.name, B.address, B.city, B.state, B.postal_code, B.latitude, B.longitude, B.review_count, B.stars, MAX(CASE WHEN H.hours LIKE "%monday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%') END) AS monday_hours, MAX(CASE WHEN H.hours LIKE "%tuesday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%')  END) AS tuesday_hours,MAX(CASE WHEN H.hours LIKE "%wednesday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%')  END) AS wednesday_hours, MAX(CASE WHEN H.hours LIKE "%thursday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%') END) AS thursday_hours, MAX(CASE WHEN H.hours LIKE "%friday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%')  END) AS friday_hours,MAX(CASE WHEN H.hours LIKE "%saturday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%') END) AS saturday_hours, MAX(CASE WHEN H.hours LIKE "%sunday%" THEN TRIM(H.hours,'%MondayTuesWednesThursFriSatSun|%') END) AS sunday_hours, GROUP_CONCAT(DISTINCT(C.category)) AS categories, GROUP_CONCAT(DISTINCT(A.name)) AS attributes, .is_open
FROM business B INNER JOIN hours H ON B.id = H.business_id INNER JOIN category C ON B.id = C.business_id INNER JOIN attribute A ON B.id = A.business_id 
GROUP BY B;
